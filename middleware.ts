...
export async function middleware(request: NextRequest) {
  ...
  if (user && pathname.startsWith("/company") && !companyRoles.has(role || "") && !adminRoles.has(role || "")) {
    return NextResponse.redirect(new URL(dashboardForRole(role), request.url));
  }

  if (user && pathname.startsWith("/customer") && (adminRoles.has(role || "") || companyRoles.has(role || ""))) {
    return NextResponse.redirect(new URL(dashboardForRole(role), request.url));
  }
