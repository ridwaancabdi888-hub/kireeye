...
export default async function HomePage(){
  const locale=await getLocale();const t=publicTranslations[locale];const vehicles=(await getAvailableVehicles()).slice(0,3);
  return <><Navbar/><main>
    <section className="hero"><div className="container"><div className="hero-grid"><div><span className="eyebrow">{t.region}</span><h1>{t.hero}</h1><p className="lead">{t.heroBody}</p></div><div className="hero-card"><span className="badge">{t.verified}</span><h2>{t.trust}</h2><div className="hero-car">🚙</div><div className="hero-stat"><strong>250+</strong><br/><span>{t.cars}</span></div></div></div>
