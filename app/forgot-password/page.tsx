...
export default function ForgotPasswordPage(){
  ...
  async function submit(e:FormEvent<HTMLFormElement>){e.preventDefault();setLoading(true);const form=new FormData(e.currentTarget);try{const supabase=createSupabaseBrowserClient();const{error}=await supabase.auth.resetPasswordForEmail(String(form.get("email")),{redirectTo:`${window.location.origin}/reset-password`});if(error)throw error;setMessage("Link-ga password reset-ka email-kaaga ayaa loo diray.");}catch(error){setMessage(error instanceof Error?error.message:"Request-ku wuu fashilmay.");}finally{setLoading(false)}}
